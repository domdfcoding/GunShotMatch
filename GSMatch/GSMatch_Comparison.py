...
class GSMCompare(object):
	...
	def setup_data(self):
		...
		chart_data = pandas.concat(
			[pandas.read_csv(os.path.join(self.Config.CSV_DIRECTORY, "{}_CHART_DATA.csv".format(self.left_sample)),
							 sep=";", index_col=0),
			 pandas.read_csv(os.path.join(self.Config.CSV_DIRECTORY, "{}_CHART_DATA.csv".format(self.right_sample)),
							 sep=";", index_col=0)
			 ], axis=1, sort=False)
		...
		for compound in chart_data.index.values:
			chart_data["Count"] = chart_data.apply(df_count, args=(
				[f"{sample} Peak Area" for sample in [self.left_sample, self.right_sample]],),
												   axis=1)
	...
	def setup_charts(self, display=False, styles=bw_default_styles, bw_colours=bw_default_colours,
					 colours=default_colours, filetypes=default_filetypes):
		if display:
			self.radar_mode = "display"
			self.box_whisker_mode = "display"
			self.pca_mode = "display"
		else:
			self.radar_mode = os.path.join(self.Config.CHARTS_DIRECTORY, "Comparison", f"{self.comparison_name}_RADAR")
			self.box_whisker_mode = os.path.join(self.Config.CHARTS_DIRECTORY, "Comparison", f"{self.comparison_name}_BOX_WHISKER")
			self.pca_mode = os.path.join(self.Config.CHARTS_DIRECTORY, "Comparison", f"{self.comparison_name}_PCA")
	...
	def radar_chart(self, figsize=(4, 9), use_log=10):
		from utils.charts import radar_chart_wrapper
		
		radar_chart_wrapper(self.chart_data, [self.left_sample, self.right_sample], use_log=use_log,
							colours=self.colours, figsize=figsize, mode=self.radar_mode, filetypes=self.filetypes)
	
	def box_whisker_chart(self, figsize=None, show_outliers=True, show_raw_data=False, err_bar="range",
						  outlier_mode="2stdev", leg_cols=1, column_width=4):
		from utils.charts import box_whisker_wrapper
		
		box_whisker_wrapper(self.chart_data, [(self.left_sample, self.left_prefixList),
											  (self.right_sample, self.right_prefixList)],
							show_outliers=show_outliers, show_raw_data=show_raw_data, err_bar=err_bar,
							outlier_mode=outlier_mode, leg_cols=leg_cols, mode=self.box_whisker_mode, figsize=figsize,
							column_width=column_width, styles=self.bw_styles, colours=self.bw_colours,
	...
	def pca(self, figsize=(8,8)):
		...
		import pandas
		from utils.charts import PrincipalComponentAnalysis
		
		pca_data = {"target": [self.left_sample] * len(self.left_prefixList) + [self.right_sample] * len(self.right_prefixList)}
		features = []
	...
	def peak_comparison(self, a_value=0.05):
		...
		while True:
			try:
				...
				outputCSV.write(f"Explained Variance Ratio: {rounders(self.pca[0],'0.0000')}, {rounders(self.pca[1],'0.0000')};;;;{self.left_sample};;;;;;;;{self.right_sample};;;;;;;;t-tests;;;;;;;;\n")
				outputCSV.write(
		...
		"""Peak Data"""
		left_peak_data = []
		with open(os.path.join(self.Config.CSV_DIRECTORY, "{}_peak_data.json".format(self.left_sample)), "r") as jsonfile:
			for peak in jsonfile.readlines():
				left_peak_data.append(json.loads(peak))
				
		right_peak_data = []
		with open(os.path.join(self.Config.CSV_DIRECTORY, "{}_peak_data.json".format(self.right_sample)), "r") as jsonfile:
			for peak in jsonfile.readlines():
	...
	#	print(score_matrix(left_A1, right_A2, Dw))
		...
		if not rt_alignment.empty:
			...
			rt_alignment[self.left_sample] = rt_alignment.apply(df_mean,
														   args=([prefix for prefix in self.left_prefixList],),
														   axis=1)
			rt_alignment[self.right_sample] = rt_alignment.apply(df_mean,
	...
	def make_archive(self):
		...
		tar = tarfile.open(os.path.join(self.Config.RESULTS_DIRECTORY, (
				self.comparison_name + "_COMPARISON_" + datetime.datetime.now().strftime("%Y%m%d%H%M%S") + '.tar.gz')),
						   mode='w:gz')
		
		for sample in [self.left_sample, self.right_sample]:
			tar.add(os.path.join(self.Config.RESULTS_DIRECTORY, sample + "_FINAL.xlsx"), arcname=(sample + ".xlsx"))
			tar.add(os.path.join(self.Config.CHARTS_DIRECTORY, sample), arcname=f'Charts/{sample}')
			tar.add(os.path.join(self.Config.SPECTRA_DIRECTORY, sample), arcname=f'Spectra/{sample}')
			
			tar.add(os.path.join(self.Config.CSV_DIRECTORY, sample + "_MERGED.csv"), arcname=sample + '_Merged.CSV')
			tar.add(os.path.join(self.Config.CSV_DIRECTORY, sample + "_MATCHES.csv"), arcname=sample + '_Matches.CSV')
			tar.add(os.path.join(self.Config.CSV_DIRECTORY, "{}_STATISTICS_FULL.csv".format(sample)),
					arcname=f'Statistics/{sample}_Statistics_Full.CSV')
			tar.add(os.path.join(self.Config.CSV_DIRECTORY, "{}_STATISTICS.csv".format(sample)),
					arcname=f'Statistics/{sample}_Statistics.CSV')
			tar.add(os.path.join(self.Config.CSV_DIRECTORY, "{}_STATISTICS_LIT.csv".format(sample)),
					arcname=f'Statistics/{sample}_Statistics_Lit.CSV')
			tar.add(os.path.join(self.Config.CSV_DIRECTORY, "{}_peak_data.json".format(sample)),
					arcname=f"{sample}_Peak_Data.json")
			tar.add(os.path.join(self.Config.RESULTS_DIRECTORY, f"{sample}.info"), arcname=f"{sample}.info")
		
		for prefix in self.left_prefixList + self.right_prefixList:
			tar.add(os.path.join(self.Config.EXPERIMENTS_DIRECTORY, "{}_tic.dat".format(prefix)),
					arcname="Experiments/{}_tic.dat".format(prefix))
			tar.add(os.path.join(self.Config.EXPERIMENTS_DIRECTORY, "{}_peaks.dat".format(prefix)),
					arcname="Experiments/{}_peaks.dat".format(prefix))
			tar.add(os.path.join(self.Config.EXPERIMENTS_DIRECTORY, "{}.expr".format(prefix)),
					arcname="Experiments/{}_expr.dat".format(prefix))
			
			tar.add(os.path.join(self.Config.CSV_DIRECTORY, "{}_COMBINED.csv".format(prefix)),
					arcname="Combined/{}.CSV".format(prefix))
		
		tar.add(os.path.join(self.Config.RESULTS_DIRECTORY, f"{self.comparison_name}_COMPARISON.CSV"), arcname="Comparison.CSV")
		tar.add(os.path.join(self.Config.RESULTS_DIRECTORY, f"{self.comparison_name}_COMPARISON.xlsx"), arcname="Comparison.xlsx")
		tar.add(os.path.join(self.Config.CHARTS_DIRECTORY, "Comparison"), arcname='Charts/Comparison')

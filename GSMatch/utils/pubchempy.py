...
class Compound(object):
				...
				for section in record["Section"]:
					if section["TOCHeading"] == "Computed Properties":
						...
						for physical_property in section["Section"]:
							...
							if (name in self._physical_properties) or (name in ["Molecular Formula", "Molecular Weight", "Physical Description"]):
								continue
							...
							try:
								...
								value = property_format(physical_property["Information"][0]["Value"]["StringWithMarkup"][0]["String"])
								if value == "Yes":
									value = True
								elif value == "No":
									value = False
	...
	@property
	def compound_is_canonicalized(self):

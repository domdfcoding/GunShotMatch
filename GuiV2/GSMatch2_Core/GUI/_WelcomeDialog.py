...
class _WelcomeDialog(wx.Dialog):
	...
	def OnOpenProject(self, event):  # wxGlade: _WelcomeDialog.<event_handler>
		...
		filename_list = utils.open_project_dialog(self)
	...
	def OnOpenInFiles(self, _):
		print("Open in Files")

		directory = str(self.recent_project_ctrl.GetFirstSelected().filename.parent).rstrip("/")
		webbrowser.open_new(directory)
	
	def OnRemoveRecent(self, _):
		print("RemoveRecent")
	...
	def on_toggle_show_dialog(self, _):  # wxGlade: _WelcomeDialog.<event_handler>
...
class RecentProjectItem(PanelListItem):
	def __init__(
		...
		with path(GuiV2.GSMatch2_Core.GUI, "RecentProjects.css") as css_path:
			css_file = str(css_path)
		
		if wx.Platform == "__WXMSW__":
			remove_btn_size = (-1, -1)
		# 	css_file = "C:\\Users\\dom13\\GunShotMatch\\GunShotMatch\\GuiV2\\GSMatch2_Core\\GUI\\RecentProjects.css"
		else:

...
class SettingsPanel(wx.Panel):
	...
	def __init__(self, parent, frame):
		...
		self._border_size.SetValue(frame.get_dock_art().GetMetric(aui.AUI_DOCKART_PANE_BORDER_SIZE))
		self._sash_size.SetValue(frame.get_dock_art().GetMetric(aui.AUI_DOCKART_SASH_SIZE))
		self._caption_size.SetValue(frame.get_dock_art().GetMetric(aui.AUI_DOCKART_CAPTION_SIZE))
	...
	def UpdateColors(self):
		...
		bk = self._frame.get_dock_art().GetColour(aui.AUI_DOCKART_BACKGROUND_COLOUR)
		self._background_color.SetBitmapLabel(self.CreateColorBitmap(bk))

		cap = self._frame.get_dock_art().GetColour(aui.AUI_DOCKART_INACTIVE_CAPTION_COLOUR)
		self._inactive_caption_color.SetBitmapLabel(self.CreateColorBitmap(cap))

		capgrad = self._frame.get_dock_art().GetColour(aui.AUI_DOCKART_INACTIVE_CAPTION_GRADIENT_COLOUR)
		self._inactive_caption_gradient_color.SetBitmapLabel(self.CreateColorBitmap(capgrad))

		captxt = self._frame.get_dock_art().GetColour(aui.AUI_DOCKART_INACTIVE_CAPTION_TEXT_COLOUR)
		self._inactive_caption_text_color.SetBitmapLabel(self.CreateColorBitmap(captxt))

		acap = self._frame.get_dock_art().GetColour(aui.AUI_DOCKART_ACTIVE_CAPTION_COLOUR)
		self._active_caption_color.SetBitmapLabel(self.CreateColorBitmap(acap))

		acapgrad = self._frame.get_dock_art().GetColour(aui.AUI_DOCKART_ACTIVE_CAPTION_GRADIENT_COLOUR)
		self._active_caption_gradient_color.SetBitmapLabel(self.CreateColorBitmap(acapgrad))

		acaptxt = self._frame.get_dock_art().GetColour(aui.AUI_DOCKART_ACTIVE_CAPTION_TEXT_COLOUR)
		self._active_caption_text_color.SetBitmapLabel(self.CreateColorBitmap(acaptxt))

		sash = self._frame.get_dock_art().GetColour(aui.AUI_DOCKART_SASH_COLOUR)
		self._sash_color.SetBitmapLabel(self.CreateColorBitmap(sash))

		border = self._frame.get_dock_art().GetColour(aui.AUI_DOCKART_BORDER_COLOUR)
	...
	def OnSashSize(self, event):
		...
		self._frame.get_dock_art().SetMetric(aui.AUI_DOCKART_SASH_SIZE,
										   event.GetInt())
		self._frame.DoUpdate()
	...
	def OnSetColor(self, event):
		...
		self._frame.get_dock_art().SetColor(var, dlg.GetColourData().GetColour())
		self._frame.DoUpdate()
		self.UpdateColors()

...
samples_to_compare = [
	...
	("GECO_SUBTRACT_20180214200701","Geco Rifle"),
...
#			print ';'.join(map(str,temp_output))
	...
	#			print outlier_list, x_val
				if type(outlier_list) == list:
						for outlier in outlier_list:
							plt.scatter(x_val, outlier, color = "k", marker = "x", label = '', zorder=3)
				else:
					plt.scatter(x_val, outlier_list, color = "k", marker = "x", label = '', zorder=3)
	...
	#			print extremes_list, x_val
				if type(extremes_list) == list:
						for extreme in extremes_list:
							plt.scatter(x_val, extreme, color = "b", marker = "*", label = '', zorder=3)
				else:
					plt.scatter(x_val, outlier_list, color = "k", marker = "*", label = '', zorder=3)
			...
			for samplex, sampleys in zip(x_vals, raw_data):
				if len(sampleys) > 0:
					for sampley in sampleys:
						plt.scatter(samplex, sampley, color = "grey", marker = "x", label = '', alpha=0.3, zorder=3)
		
		if use_mean:
			# mean and stdev
			#if mark == 'D':
			plt.scatter(x_vals, data, color = colour, marker=mark, label = compound, zorder=3,s=20)
			#else:
			#	plt.scatter(x_vals, data, color = colour, marker=mark, label = compound, zorder=3)
			plt.errorbar(x_vals, data,yerr=data_Err, linestyle="None", color = colour, capsize=3, zorder=3)

		elif use_median:
			# median and IQR
			plt.scatter(x_vals, data, color = colour, marker=mark, label = compound, zorder=100)
			#print data_Err
	...
	plt.grid(b=True, axis = "y", which='major', color='b', linestyle='-',zorder=1)
	plt.grid(b=True, axis = "y", which='minor', color='r', alpha=0.3, linestyle='--',zorder=1)
	...
	if mode == "singles":
		...
		ax.xaxis.set_minor_formatter(ticker.FixedFormatter(["Unfired\nPropellant","Fired\nCartridge Case"]*(len(samples_to_compare)/2)))
	...
	if mode == "pairs":
		...
		# Thick grilines
		par1 = ax.twiny()
		par1.set_xlim(left=x_vals[0]-(shift/2), right=(x_vals[-1]+(shift/2)))
		
		par1.set_xticks(top_vals)		

		par1.set_xticklabels(['']*len(top_vals))
		par1.grid(axis = "x", which='major', linestyle='-', linewidth=1.2, zorder = 1, color="k")
		par1.tick_params(axis='x', colors="k", size=20, width=1.2)		
		
		par1.set_axisbelow(True)
		
		# Labels above graph
		par2 = ax.twiny()
		par2.set_xlim(left=x_vals[0]-(shift/2), right=(x_vals[-1]+(shift/2)))
		
		par2.set_xticks(min_top_vals)
	
		top_label_list = []
		for ID in IDs[::2]:
			top_label_list.append(ID)
			#top_label_list.append("") 	
	
		par2.set_xticklabels(top_label_list)
		tkw = dict(size=4, width=1.5)
		par2.tick_params(axis='x', colors="k", **tkw)
		#par2.grid(axis = "x", which='major', linestyle='-', linewidth=2)
		par2.set_axisbelow(True)
		par2.set_xlabel("Ammunition",fontsize=12,labelpad=10)
		
		

	fig.set_size_inches(11.3,8.27)
...
#	for file_format in ["pdf"]:
		plt.savefig("./charts/{}_{}_groups_chart.{}".format("_".join(IDs),metric,file_format), papertype = 'a4', orientation = 'landscape', format = file_format, dpi = 1200,bbox_inches="tight")
	...
	for file_format in ["pdf","png"]:
#	for file_format in ["pdf"]:
		plt.savefig("./charts/{}_{}_groups_legend.{}".format("_".join(IDs),metric,file_format), format = file_format, dpi = 1200,bbox_inches="tight")
